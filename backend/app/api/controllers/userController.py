...
def create_user(db: Session, user_data: UserCreate):
    ...
    existing_user = db.query(UserModel).filter(UserModel.user_email == user_data.user_email).first()
    if existing_user:
    ...
    for role_name in user_data.roles:
        ...
        db_role = db.query(RoleModel).filter(RoleModel.role_name.ilike(role_name)).first()
