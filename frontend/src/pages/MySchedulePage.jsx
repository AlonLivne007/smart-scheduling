/**
 * My Schedule Page - Employee view of their assigned shifts
 * 
 * Displays published schedules and shifts assigned to the current employee.
 * Shows shifts in a weekly calendar format with shift details.
 */
...
export default function MySchedulePage() {
  ...
  async function loadMyShifts() {
    ...
    try {
      ...
      const shiftsWithDetails = await Promise.all(
        assignments.map(async (assignment) => {
          try {
            ...
            // Fetch the schedule to check if it's published
      ...
      // Filter out nulls and only keep PUBLISHED schedules
        ...
        <Card>
          <div className="p-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <div className="text-gray-600">Days Working</div>
                <div className="text-2xl font-bold text-blue-600">
                  {new Set(myShifts.map(s => s.shift.date)).size}
                </div>
              </div>
            </div>
          </div>
